...
namespace NightlyCode.Database.Tests.Entities {
    ...
    public class EntityManagerTest {
        ...
        [Test, Parallelizable]
        public void MultipleLoadsInParallel() {
            IDBClient dbclient = TestData.CreateDatabaseAccess();
            EntityManager entitymanager = new EntityManager(dbclient);
            entitymanager.Create<TestEntityWithoutAnySpecifications>();
            ...
            TestEntityWithoutAnySpecifications[] source = TestEntities.ToArray();
            entitymanager.InsertEntities<TestEntityWithoutAnySpecifications>().Execute(source);
            IEnumerable<TestEntityWithoutAnySpecifications> loaded = entitymanager.Load<TestEntityWithoutAnySpecifications>()
                .Where(t => t.Something < 8.0).ExecuteEntities<TestEntityWithoutAnySpecifications>();
            IEnumerable<TestEntityWithoutAnySpecifications> loaded2 = entitymanager.Load<TestEntityWithoutAnySpecifications>()
                .ExecuteEntities<TestEntityWithoutAnySpecifications>();
            foreach (TestEntityWithoutAnySpecifications entity in loaded2)
        ...
        [Test, Parallelizable]
        public void DBFieldComparisionBool() {
            IDBClient dbclient = TestData.CreateDatabaseAccess();
            EntityManager entitymanager = new(dbclient);
            entitymanager.Create<TestEntityWithoutAnySpecifications>();
            entitymanager.Update<TestEntityWithoutAnySpecifications>().Set(t => t.Column1 == "bla")
                         .Where(t => DB.Property<TestEntityWithoutAnySpecifications>(e=>e.BooleanValue).Bool == true)
                         .Execute();
        }

        [Test, Parallelizable]
        public void DBFieldComparisionArbitraryType() {
            IDBClient dbclient = TestData.CreateDatabaseAccess();
            EntityManager entitymanager = new(dbclient);
            entitymanager.Create<TestEntityWithoutAnySpecifications>();
            entitymanager.Update<TestEntityWithoutAnySpecifications>().Set(t => t.Column1 == "bla")
                         .Where(t => DB.Property<TestEntityWithoutAnySpecifications>(e=>e.Something).Type<double>() == 0.0)
                         .Execute();
        }
        ...
        [Test, Parallelizable]
        public void LikeOperatorStatic() {
            IDBClient dbclient = TestData.CreateDatabaseAccess();
            EntityManager entitymanager = new(dbclient);
            entitymanager.Create<TestEntityWithoutAnySpecifications>();
            entitymanager.Update<TestEntityWithoutAnySpecifications>().Set(t => t.Column1 == "123").Where(t => DBOperators.Like(t.Column1, "%something%")).Execute();
        }

        [Test, Parallelizable]
        public void LikeOperatorStaticUsingPredicateExpression() {
            IDBClient dbclient = TestData.CreateDatabaseAccess();
            EntityManager entitymanager = new(dbclient);
            entitymanager.Create<TestEntityWithoutAnySpecifications>();
            PredicateExpression<TestEntityWithoutAnySpecifications> predicate = null;
            predicate &= t => DBOperators.Like(t.Column1, "%something%");
            entitymanager.Update<TestEntityWithoutAnySpecifications>().Set(t => t.Column1 == "123").Where(predicate?.Content).Execute();
        }
        ...
        public void Length() {
            ...
            EntityManager entitymanager = new(dbclient);
            entitymanager.Create<TestEntityWithoutAnySpecifications>();
            entitymanager.Load<TestEntityWithoutAnySpecifications>().Where(t => t.Column1.Length > 2).Execute();
        }
