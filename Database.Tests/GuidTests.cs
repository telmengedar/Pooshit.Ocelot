...
namespace NightlyCode.Database.Tests {
    ...
    public class GuidTests {
        ...
        public void FindGuids() {
            ...
            database.Insert<GuidEntity>().Columns(g => g.Guid, g => g.SomeValue).Values(guid, 7).Execute();
            GuidEntity result=database.LoadEntities<GuidEntity>().Where(g => g.Guid == guid).Execute().FirstOrDefault();
