...
namespace Pooshit.Ocelot.Entities.Descriptors {
    ...
    public class EntityDescriptorAccess<T> {
        ...
        readonly PropertyVisitor propertyvisitor = new();
        ...
        public EntityDescriptorAccess<T> Index(string name, params Expression<Func<T, object>>[] columns) {
            descriptor.AddIndex(new IndexDescriptor(name, columns.Select(visitor.GetColumnName), null));
            return this;
        }
        ...
        /// <param name="columns">columns to be included in index</param>
        public EntityDescriptorAccess<T> Index(string name, string type, params Expression<Func<T, object>>[] columns) {
            descriptor.AddIndex(new IndexDescriptor(name, columns.Select(visitor.GetColumnName), type));
            return this;
        }
        ...
        public EntityDescriptorAccess<T> Unique(params Expression<Func<T, object>>[] columns)
        {
            ...
            if (columns.Length > 1)
            {
                descriptor.AddUnique(new UniqueDescriptor(null, columns.Select(visitor.GetColumnName)));
            }
            else
            {
